...
class WideResNet(nn.Module):
    def __init__(self, depth, widen_factor=1, dropRate=0.0, num_classes=10, dim_aux=0):
    ...
    def forward(self, x, return_feature=False, return_aux=False, use_cam=False):
        ...
        out = self.block2(out)
        out = self.block3(out)
        featconv = self.relu(self.bn1(out))
        out = F.avg_pool2d(featconv, 8)
...
def WideResNet40_2(num_classes=11, **kwargs):
    logging.info("Loading Wide-ResNet40-2")
