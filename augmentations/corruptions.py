...
imsize = 224 # for imagenet
# imsize = 32 # for cifar10/100, svhn
# imsize = 54 # for tiny-imagenet
...
def plasma_fractal(mapsize=imsize, wibbledecay=3):
    """
...
def fog(x, severity=1):
    ...
    x += c[0] * plasma_fractal(wibbledecay=c[1])[:imsize, :imsize][..., np.newaxis]
...
def frost(x, severity=1):
    ...
    filename = ['./figures/frost1.png', './figures/frost2.png', './figures/frost3.png', './figures/frost4.jpg', './figures/frost5.jpg', './figures/frost6.jpg'][idx]
    frost = cv2.imread(filename)
    frost = cv2.resize(frost, (0, 0), fx=0.2, fy=0.2)
    # randomly crop and convert to rgb
    x_start, y_start = np.random.randint(0, frost.shape[0] - imsize), np.random.randint(0, frost.shape[1] - imsize)
    frost = frost[x_start:x_start + imsize, y_start:y_start + imsize][..., [2, 1, 0]]
...
def snow(x, severity=1):
    ...
    x = c[6] * x + (1 - c[6]) * np.maximum(x, cv2.cvtColor(x, cv2.COLOR_RGB2GRAY).reshape(imsize, imsize, 1) * 1.5 + 0.5)
